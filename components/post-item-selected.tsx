...
export default function PostItemS({ event }: PostItemProps) {
  return (
      <article className="flex flex-col h-[390px]"> {/* 增加高度从300px到400px */}
        <header>
          {event.image && (
            <Link className="block mb-4" href={`/vision/events/${event.id}`}>
        ...
        <Link className="hover:underline" href={`/vision/events/${event.id}`}>
          <h4 className="h4 font-red-hat-display mt-3 mb-2 text-xl">{event.name}</h4>
        </Link>
