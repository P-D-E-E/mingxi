            ...
            <section>
                <nav className="bg-white w-full z-30 md:bg-opacity-90 transition duration-300 ease-in-out">
                    ...
                    <div className="flex justify-center h-16 md:h-20">
                        <ul className="flex space-x-12 text-center"> {/* 调整 space-x 值以控制空格宽度 */}
                        {navItems.map((item) => (
                            <li key={item.href}>
                            <Link
                                href={item.href}
                                className=" text-rblue-900 hover:text-hightlight px-4 lg:px-6 py-2 text-xl flex items-center transition duration-150 ease-in-out"
                            >
                                {item.label}
                            </Link>
                            </li>
